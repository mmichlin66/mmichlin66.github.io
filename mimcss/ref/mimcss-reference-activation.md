...
- [$cancelActivation Function](#cancelactivation-function)
...
#### $forceActivation() Function

```tsx
export function $forceActivation( schedulerType?: number): void
```

The `$forceActivation` function writes to DOM all style changes caused by the calls to the `$activate`, `$deactivate` and `IStyleRule.setProp` functions accumulated since the last activation of the given scheduler type. If the `schedulerType` parameter is left undefined, the current default scheduler type is used.
